    ...
    const loginModal = document.getElementById('loginModal'); // Modal de alerta
    ...
    function reservarEvento(nombre, lugar, fecha, hora, ciudad, categoria) {
      ...
      const usuarioLogueado = localStorage.getItem('usuario'); // Aquí podemos usar sessionStorage si prefieres esa opción

      if (usuarioLogueado) {
        // Si el usuario está logueado, se guarda la reserva y se procede
        ...
        // Si el usuario no está logueado, muestra el modal de alerta
        loginModal.style.display = 'block';

        // Cambiar el tiempo de espera para redirigir al login (por ejemplo 10 segundos)
        setTimeout(() => {
          loginModal.style.display = 'none'; // Ocultar el modal
          window.location.href = 'login.html'; // Redirige al login después de 10 segundos
        }, 5000); // 5 segundos
      }
    }
